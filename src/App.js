...
export default function App() {
  const { data, loading, setSearchURL } = useFetch(PEOPLE_PAGE_ONE);
  return (
    <>
      {loading && <Loading />}
      <div className='relative mt-16 min-h-full bg-gradient-to-t from-black via-gray-700 to-gray-200'>
        <Header>{!loading && <Navbar setSearchURL={setSearchURL} />}</Header>
